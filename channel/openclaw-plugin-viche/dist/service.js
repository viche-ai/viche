...
// ---------------------------------------------------------------------------
async function registerOnce(config) {
    const body = {
...
async function registerWithRetry(config, logger) {
    let lastError = null;
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            ...
            logger.error(`Viche: registration attempt ${attempt}/${MAX_ATTEMPTS} failed: ${lastError.message}`);
            if (attempt < MAX_ATTEMPTS) {
                await sleep(BACKOFF_MS);
            }
        }
    }
    throw new Error(`Viche: registration failed after ${MAX_ATTEMPTS} attempts: ${lastError?.message ?? "unknown error"}`);
}
// ---------------------------------------------------------------------------
 ...
 */
function resolveSessionKey(payload, config, state) {
 ...
 */
function cleanupExpiredCorrelations(state) {
...
// ---------------------------------------------------------------------------
async function handleInboundMessage(payload, runtime, config, state, logger) {
    ...
    if (typeof payload.id !== "string" || payload.id === "" ||
        ...
        throw new Error(`Viche: received inbound message with missing or invalid required fields: ${JSON.stringify(payload)}`);
    }
    ...
    try {
        ...
        logger.info(`Viche: injected message ${payload.id} from ${payload.from} into session ${sessionKey} (runId: ${runId})`);
    }
    catch (err) {
 ...
 */
export function createVicheService(config, state, runtime, _openclawConfig) {
    let socket = null;
    let channel = null;
            ...
            const connectAndJoin = (agentId) => {
                ...
                return new Promise((resolve, reject) => {
                    channel
                        .join()
                        .receive("ok", () => {
                        logger.info(`Viche: registered as ${agentId}, connected via WebSocket`);
                        for (const token of config.registries ?? []) {
                            const registryChannel = socket.channel(`registry:${token}`, {});
                            registryChannel
                                .join()
                                .receive("error", (resp) => {
                                logger.warn(`Viche: registry channel join failed for ${token}: ${JSON.stringify(resp)}`);
                            });
                        }
                        resolve();
                    })
                        .receive("error", (resp) => {
                        reject(new Error(`Viche: channel join failed: ${JSON.stringify(resp)}`, { cause: resp }));
                    })
                        .receive("timeout", () => {
